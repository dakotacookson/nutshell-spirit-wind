...
const printedMessages = {
  ...
  messageBuilder(messageObject) {
    ...
    messageUser.textContent = messageObject.userId
    ...
    // In order to change the data for an existing message in our API, we need to provide the user with a way to edit the existing values. This means we will show the user a form with the existing values already populated. Once again, we want our data to be our point of truth. So we make a HTTP GET request targeting the specific food item the user wants to edit to get the data that will be populated in the form. Once we have that data, we can build the form, populate the input fields with our data form the GET request and then append that form to the appropriate place on the DOM.


    messageArticle.appendChild(messageUser)
  ...
  postMessage(){
    ...
    .then(allMessages => {
      ...
      allMessages.forEach(message => {
        let messageHtml = printedMessages.messageBuilder(message)
